...
app.post("/rateSeason", async (req, res) => {
  try {
    ...
    res
      .status(500)
      .json({ message: "Error updating season rating and saving activity" });
  }
});
...
app.post("/delSeason", async (req, res) => {
  try {
    ...
    if (deletionResultActivity.deletedCount === 0) {
      return res
        .status(404)
        .json({ message: "Activity not found for the given user and show" });
    }
...
app.delete(
  "/api/activities/:activityId/comment/:commentId",
  async (req, res) => {
    try {
      const activityId = req.params.activityId;
...
app.post(
  "/api/activities/comment/:commentId/reply/:replyId/like",
  async (req, res) => {
    try {
...
app.post(
  "/api/activities/comment/:commentId/reply/:replyId/unlike",
  async (req, res) => {
    try {
      const { replyId } = req.params;
      const { userId } = req.body;
